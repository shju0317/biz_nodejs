...
수없이 많은 이슈들이 있다.
JS(node)에 Date라는 내장 클래스가 있다.
이 내장클래스도 이슈가 있어서 실제로 DB와 연동하여 사용할 때
문제들을 일으킨다.
...
router.get("/list", function (req, res) {
  ...
  bbsVO.find().then(function (bbsList) {
    ...
    // rendering을 수행할 때 bbsList파일을 ModelAttribute 형식으로 담아서 전송한다.
    // 현재버진인 14.x 버전에서는 { bbsList } 라고 표현하면
...
router.get("/write", function (req, res) {
  // bbsWrite.pug파일을 rendering하여 요청 전송
  // insert와 update를 write.pug 파일로 공통으로 사용하기 위해서
  // insert를 수행할 때 비어있는 vo를 만들어서 bbsWrite에 전달해줘야 한다.
  let data = new bbsVO();
...
router.post("/write", function (req, res) {
  ...
  // 하나의 vo객체로 생성을 하고
  // 거기에다가 DB CRUD와 관련된 함수들을 함께 추가하여 객체를 생성한다.
...
// localhost:3000/bbs/view/i값 uRL 요청
...
조건문 없이 remove()를 실행하면 테이블의 모든 데이터를 바로 삭제해버린다.
mongoDB 4.x 이상에서는 이러한 문제를 방지학 위해서
...
router.get("/delete/:id", function (req, res) {
  ...
  findOneAndDelete(조건문) : 삭제한 후 삭제된 데이터를 다시 확인하고 싶을 때
...
// 전달받은 PK 값으로 한 개의 item을 조회하고
// write form에 보여주고
// 데이터를 입력한 다음 저장을 하면
// post("/update")로 보내서 데이터를 수정하도록 한다.
...
/*
한개의 item을 조회하여 write for으로 보낸 후 데이터를 다시 받았을 때
...
params로부터 id를 추출하여 강제로 req.body에 추가해준다.
  ...
  bbsVO
    .updateOne(
      { _id: id }, // where 조건문
      {
        ...
        // 주의해야 할 코드
        // $set{req.body}
        // update를 수행하는 데 실제로 form에서 전달된 데이터 외에
